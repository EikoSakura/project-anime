...
- Monster Creator rebuilt around the EXP purchase: Tier tiles with a Party XP pull, Weapon / Armor / Shield Style pickers stamped as real equipped gear, attribute and box spending with a live EXP budget pill, and the review statblock in the printed format
- Villains: Threat = the full encounter budget with a free Retinue, and three recorded Luck Dice like a PC (a GM with no assigned character spends from the Villain pool on its cards)
- Gates replace Boss Bars: purchased hit boxes divide across ceil(party/2) Gates with no hit-box cap while gated, the Villain acts twice per Enemy Phase, two Techniques per Gate with real locks (passives included), and a break loses excess damage, clears detrimental statuses, and unlocks the next Gate
- Encounter Budget: party size x difficulty (Easy 1/2, Standard 1, Hard 2, Climax 3) across the Party planner and the HUD Threat readout; migration enemyTiersV1 folds the old Types into the Tier ladder and converts Bosses/Rivals into Villains (Bars carry over as Gates)

## 0.6.0 - 2026-07-14

- Enemies rebuilt on the printed Tier x EXP rules: five Tiers (Minion 1/5, Standard 2/10, Elite 3/15, Champion 4/20, Villain full-budget/30 - Threat / base EXP), every enemy starting from the shared base line (1 hit box, 1 energy box, all Attributes d4, Guard 6 + Armor Style + Shield) and spending base EXP + the party's earned XP on the printed purchase list (+1 Attribute step 1, +1 hit/energy box 1, Talent at d6 2, Step Up a Talent 1, Technique 2, Villain Luck step 2); spends are derived live from the built statblock - no ledger
- Monster Creator rebuilt around the EXP purchase: Tier tiles (Threat + base EXP on hover) with a Party XP field (pull from the roster's earned advancements), Weapon / Armor / optional Shield Style pickers off the player tables (stamped as real equipped items, so Guard and Movement derive exactly like a PC's), attribute + box spending with a live EXP budget pill, and the review statblock in the printed format (styles line, Talents with dice, Techniques, Villain Luck + Gates)
- Villains: one enemy built to challenge the whole party - Threat = the full encounter budget, its Retinue costs nothing; records three Luck Dice like a PC (base d6, stepped up with EXP) - a GM without an assigned character spends them from the Villain's own pool on its cards, and Go-Again restore/tune works for Villains too
- Gates (replacing Boss Bars): a climax Villain's hit boxes divide across ceil(party/2) Gates (no hit-box cap while gated); energy stays one pool; it acts twice per Enemy Phase; two Techniques per Gate, enforced - Techniques carry a Gate assignment and stay locked until their Gate opens; breaking a Gate loses excess damage, clears every detrimental status, and unlocks the next Gate's Techniques
- Encounter Budget: party size x difficulty (Easy 1/2, Standard 1, Hard 2, Climax 3); Minions still cap at half the budget; the Party planner and the HUD Threat readout both speak the new costs, show a Villain as Full Budget, and mark its Retinue free
- Migration enemyTiersV1: retired Types fold into the Tier ladder (Bruiser -> Elite, Skirmisher/Support -> Standard), Bosses become gated Villains (Bars carry over as Gates 1:1), Rivals become Villains; the retired boss/rival data drops from world and unlocked-pack NPCs. NOTE: the bundled NPC compendium still holds old-line stat numbers - rebuild those enemies with the new Creator when convenient
