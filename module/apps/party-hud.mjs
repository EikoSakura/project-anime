/* The Party HUD: one status card per player character, docked over the
   left edge of the board on every client and open through play. Heart,
   Energy, and Luck controls write only on actors the client owns; the
   Grade plate flip is per-client display state; every card renders the
   same everywhere. */
...
export function registerPartyHud() {
  ...
  for (const hook of ["createActor", "updateActor", "deleteActor"]) Hooks.on(hook, refresh);
...
class PartyHud extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  /* Character actors with a player owner, in actors directory order:
     each folder branch lists its subfolders' entries, then its own. */
  static #partyActors() {
    const out = [];
    const walk = node => {
      for (const child of node.children) walk(child);
      for (const entry of node.entries) {
        if (entry.type === "character" && entry.hasPlayerOwner) out.push(entry);
      }
    };
    walk(game.actors.tree);
    return out;
  }
